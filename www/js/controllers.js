...
.controller('introPageCtrl',function($scope,$http,$state){
	setTimeout(function(){
		$state.go('app.wordpress');
	},7000);
	//$state.go('auth.walkthrough');
...
.controller('HomeCtrl', function($scope, $rootScope, BookMarkService,$ionicLoading, $state) {


/*
		$ionicLoading.show({
					template: 'Loading...',duration: 0
		})
*/
	eval(mcmchartdata)

	//dmj("#chart2").append(\''+sayonarachart+'\');
	//dmj("#chart2").bind("mouseover",function(){dmj("#sayonarachart").remove()});
...
.controller('notebookCtrl', function ($scope, $stateParams) {
    if ($stateParams.id != null) {
		$scope.title = regRightData[$stateParams.id][0];
		$scope.datas = regRightData[$stateParams.id][1];
...
.controller('calcCtrl', function ($scope, $stateParams) {
    if ($stateParams.id != null) {
		$scope.title = mydatas2[$stateParams.id][0];
		$scope.number = mydatas2[$stateParams.id][1];
		$scope.money = mydatas2[$stateParams.id][2];
		//console.log(document);
	}else{
		console.log("Sorry");
	}
})
.controller('calcMainstreamCtrl', function ($scope, $stateParams) {
	eval(mcmchartdata2)
})
